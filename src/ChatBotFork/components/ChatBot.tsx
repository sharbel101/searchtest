...
const ChatBot = ({
  ...
  const finalFlow =
    !flow || Object.keys(flow).length === 0 ? WelcomeFlow : flow;
  ...
  const [finalThemes, setFinalThemes] = useState<Theme | Array<Theme>>(
    !themes ? [] : themes,
  );
  useEffect(() => {
    if (themes) {
