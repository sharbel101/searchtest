...
const ToastContainer = () => {
  ...
  const toastPromptContainerStyle: CSSProperties = {
    bottom: `calc(${
      typeof styles.footerStyle?.height === 'number'
        ? `${styles.footerStyle?.height}px`
        : (styles.footerStyle?.height ?? '50px')
    } + ${
      typeof styles.chatInputContainerStyle?.height === 'number'
        ? `${styles.chatInputContainerStyle?.height}px`
        : (styles.chatInputContainerStyle?.height ?? '70px')
    } + 20px)`,
    width: 300,
    minWidth: `calc(${
      typeof styles.chatWindowStyle?.width === 'number'
        ? `${styles.chatWindowStyle?.width}px`
        : (styles.chatWindowStyle?.width ?? '375px')
    } / 2)`,
    maxWidth: `calc(${
      typeof styles.chatWindowStyle?.width === 'number'
        ? `${styles.chatWindowStyle?.width}px`
  ...
  return (
    <div
      ...
      style={toastPromptContainerStyle}
    >
      {toasts.map((toast) => (
        <ToastPrompt
          key={toast.id}
