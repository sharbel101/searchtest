...
const BotStatesContext = createContext<BotStatesContextType>(
  {} as BotStatesContextType,
);
...
const BotStatesProvider = ({
  ...
  const [isBotTyping, setSyncedIsBotTyping, syncedIsBotTypingRef] =
    useSyncedRefState<boolean>(false);
  const [
    isChatWindowOpen,
    setSyncedIsChatWindowOpen,
    syncedIsChatWindowOpenRef,
  ] = useSyncedRefState<boolean>(settings?.chatWindow?.defaultOpen ?? false);
  const [audioToggledOn, setSyncedAudioToggledOn, syncedAudioToggledOnRef] =
    useSyncedRefState<boolean>(settings?.audio?.defaultToggledOn ?? false);
  const [voiceToggledOn, setSyncedVoiceToggledOn, syncedVoiceToggledOnRef] =
    useSyncedRefState<boolean>(settings?.voice?.defaultToggledOn ?? false);
  const [
  ...
  const [isLoadingChatHistory, setIsLoadingChatHistory] =
    useState<boolean>(false);
  const [hasChatHistoryLoaded, setHasChatHistoryLoaded] =
    useState<boolean>(false);
  const [isScrolling, setSyncedIsScrolling, syncedIsScrollingRef] =
    useSyncedRefState<boolean>(false);
  const [
    textAreaDisabled,
    setSyncedTextAreaDisabled,
    syncedTextAreaDisabledRef,
  ] = useSyncedRefState<boolean>(true);
  const [
  ...
  const [blockAllowsAttachment, setBlockAllowsAttachment] =
    useState<boolean>(false);
  // tracks view port height and width (for auto-resizing on mobile view)
  const [viewportHeight, setViewportHeight] = useState<number>(
    (window.visualViewport?.height as number) ?? window.innerHeight,
  );
  const [viewportWidth, setViewportWidth] = useState<number>(
    (window.visualViewport?.width as number) ?? window.innerWidth,
  );
