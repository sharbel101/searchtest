...
export const useBotEffectsInternal = () => {
    ...
    const handleUserScroll = () => {
      ...
      requestAnimationFrame(() => {
        ...
        const isScrolling =
          ...
          scrollHeight - (settings.chatWindow?.messagePromptOffset ?? 30);
  ...
  useEffect(() => {
    ...
    settings.chatHistory?.storageKey,
    settings.chatHistory?.maxEntries,
    settings.chatHistory?.disabled,
  ]);

  // handles virtualkeyboard api (if supported on browser) on mobile devices
  useEffect(() => {
    const nav = navigator as Navigator & { virtualKeyboard?: any };
    if (isDesktop || settings.general?.embedded || !nav.virtualKeyboard) {
      return;
    }

    nav.virtualKeyboard.overlaysContent = true;
    nav.virtualKeyboard.addEventListener('geometrychange', (event: any) => {
      if (!event.target) {
        return;
      }
