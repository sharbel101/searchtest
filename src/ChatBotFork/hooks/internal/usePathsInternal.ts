...
export const usePathsInternal = () => {
   ...
   */
  const firePostProcessBlockEvent = useCallback(
    async (block: Block): Promise<Block | null> => {
  ...
  const handlePathChange = useCallback(
    async (currPath: string | null, prevPath: string | null) => {
      if (!currPath) {
        return;
      }
  ...
  const getCurrPath = useCallback(() => {
    return syncedPathsRef.current.length > 0
      ? (syncedPathsRef.current.at(-1) ?? null)
      : null;
  }, []);
  ...
  const getPrevPath = useCallback(() => {
    return syncedPathsRef.current.length > 1
      ? (syncedPathsRef.current.at(-2) ?? null)
      : null;
  }, []);
  ...
  const goToPath = useCallback(
    async (pathToGo: keyof Flow): Promise<boolean> => {
