...
export const parseConfig = async (
  botId: string,
  providedSettings: Settings | undefined,
  ...
  if (themes != null) {
    if (Array.isArray(themes)) {
      for (const theme of themes) {
        ...
        combinedSettings = getCombinedConfig(
          themeConfig.settings,
          combinedSettings,
        ) as Settings;
        combinedStyles = getCombinedConfig(
          themeConfig.inlineStyles,
          combinedStyles,
        ) as Styles;
  ...
  if (
    combinedSettings.chatInput?.botDelay != null &&
...
export const getCombinedConfig = (
  preferredConfig: Settings | Styles,
  baseConfig: Settings | Styles,
): Settings | Styles => {
  const stack: Array<{ source: object; target: object }> = [
  ...
  while (stack.length) {
    ...
    for (const key of Object.keys(source)) {
      ...
      if (isValidElement(source[keyAsObjectType])) {
        ...
        if (
          typeof target[keyAsObjectType] !== 'object' ||
          target[keyAsObjectType] === null
        ) {
          (target as Record<string, object>)[keyAsObjectType] = {};
        }
        stack.push({
          source: source[keyAsObjectType],
          target: target[keyAsObjectType],
        });
      } else {
        try {
          target[keyAsObjectType] = source[keyAsObjectType];
