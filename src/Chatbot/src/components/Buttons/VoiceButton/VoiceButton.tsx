...
const VoiceButton = () => {
  ...
  const { setInputLength, setTextAreaValue, textAreaDisabled } =
    useTextAreaInternal();
  ...
  const handleSendAsAudio = async () => {
    ...
    const audioFile = new File([audioBlob], 'voice-input.wav', {
      type: 'audio/wav',
    });
   ...
   */
  const renderButton = () => {
    const IconComponent = voiceToggledOn
      ? settings.voice?.icon
      : settings.voice?.iconDisabled;
