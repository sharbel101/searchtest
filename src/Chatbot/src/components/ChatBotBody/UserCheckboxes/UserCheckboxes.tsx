...
const UserCheckboxes = ({
  ...
  const botCheckboxNextStyle: React.CSSProperties = {
    cursor:
      disabled || checkedBoxes.size < (checkboxes.min as number)
        ? `url("${settings.general?.actionDisabledIcon}"), auto`
        : 'pointer',
    color: settings.general?.primaryColor,
    borderColor: settings.general?.primaryColor,
  ...
  return (
    <div
      className={`rcb-checkbox-container ${settings.botBubble?.showAvatar ? 'rcb-checkbox-offset' : ''}`}
    >
