...
export const useNotificationInternal = () => {
  ...
  const setUpNotifications = useCallback(async () => {
    ...
    audioBufferRef.current =
      await audioContextRef.current.decodeAudioData(audioSource);
  ...
  const playNotificationSound = useCallback(() => {
    if (
      ...
      !syncedNotificationsToggledOnRef.current ||
      !hasInteractedPage
    ) {
      return;
    }
    ...
    source
      .connect(gainNodeRef.current as AudioNode)
  ...
  const toggleNotifications = useCallback(
    async (active?: boolean) => {
      ...
      if (settings.event?.rcbToggleNotifications) {
        const event = await dispatchRcbEvent(RcbEvent.TOGGLE_NOTIFICATIONS, {
