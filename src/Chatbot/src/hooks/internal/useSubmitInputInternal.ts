...
export const useSubmitInputInternal = () => {
  ...
  const { getCurrPath, getPrevPath, goToPath, firePostProcessBlockEvent } =
    usePathsInternal();
  ...
  // handles bot refs
  const { flowRef, inputRef, keepVoiceOnRef, paramsInputRef, timeoutIdRef } =
    useBotRefsContext();
        ...
        if (settings?.sensitiveInput?.hideInUserBubble) {
          ...
          if (settings.userBubble?.simulateStream) {
            await simulateStreamMessage(
              '*'.repeat(
                (settings.sensitiveInput?.asterisksCount as number) ?? 10,
              ),
              'USER',
            );
          } else {
  ...
  const handleSubmitText = useCallback(
    async (inputText?: string, sendInChat = true) => {
      ...
      if (settings.event?.rcbUserSubmitText) {
        const event = await dispatchRcbEvent(RcbEvent.USER_SUBMIT_TEXT, {
          inputText,
          sendInChat,
        });
