...
export const getCachedTheme = (
  ...
  const cachedTheme = localStorage.getItem(
    `${CACHE_KEY_PREFIX}_${id}_${version}`,
  );
 ...
 */
export const processAndFetchThemeConfig = async (
  botId: string,
  theme: Theme,
): Promise<{
  settings: Settings;
  ...
  const themeVersion = version
    ? version
    : await fetchThemeVersionFromMeta(id, (baseUrl ?? DEFAULT_URL) as string);
  ...
  const cache = getCachedTheme(
    id,
    themeVersion,
    Number(cacheDuration ?? DEFAULT_EXPIRATION),
  );
  if (cache) {
    const scopedCssText = getScopedCssStylesText(botId, cache.cssStylesText);
    return {
      settings: cache.settings,
      inlineStyles: cache.inlineStyles,
      cssStylesText: scopedCssText,
    };
  }
...
const getScopedCssStylesText = (botId: string, cssStylesText: string) => {
  const scopedCssText = cssStylesText
    .split(/(?<=})/)
    .map((rule) => {
      ...
      if (
        ...
        trimmedRule.startsWith('@keyframes') ||
