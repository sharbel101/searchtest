...
let recognition:
  | (typeof window.SpeechRecognition extends { prototype: infer T } ? T : any)
  | null;
...
const startSpeechRecognition = (
  ...
  recognition.onresult = (event: SpeechRecognitionEvent) => {
    ...
    const voiceInput: string =
      event.results[event.results.length - 1][0].transcript;

    if (inputRef.current) {
      const characterLimit: number | undefined =
        settings.chatInput?.characterLimit;
      const newInput: string = inputRef.current.value + voiceInput;
      if (
        characterLimit != null &&
        characterLimit >= 0 &&
 ...
 * @param settings options provided to the bot
 */
export const syncVoiceWithChatInput = (
  keepVoiceOn: boolean,
  settings: Settings,
) => {
  const recognition = getSpeechRecognition();
