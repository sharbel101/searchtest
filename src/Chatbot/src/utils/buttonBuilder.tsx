...
export const getButtonConfig = (
  settings: Settings,
  buttonComponentMap: { [x: string]: () => React.JSX.Element },
): ButtonConfig => {
  ...
  const buttonCache: { [key: string]: React.ReactElement } = {};

  const mapButtons = (
    buttons: Array<string | React.JSX.Element> | undefined,
  ) => {
    if (!buttons) {
      return [];
    }
    ...
    return buttons
      ...
      .filter((button) => button !== null) as React.JSX.Element[];
  };
