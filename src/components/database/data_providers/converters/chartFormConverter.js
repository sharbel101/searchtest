...
// CSV header (no id column)
const rows = [['id', 'question', 'answers']];

// Fill rows
for (const key in investmentStageFlow) {
  const q = investmentStageFlow[key];
  rows.push([
    key,
    q.question.replace(/"/g, '""'), // escape quotes in text
    JSON.stringify(q.answers).replace(/"/g, '""'), // escape for CSV
  ]);
}

// Convert rows to CSV text
const csvText = rows.map((r) => r.map((v) => `"${v}"`).join(',')).join('\n');

// Save to file
fs.writeFileSync('chartform_flow.csv', csvText);

console.log('CSV file created: chartform_flow.csv');
